...
You will then need to save the PTBPs as a BED file using `write.table` as shown below. Once saved, import them into Juicebox using `Show Annotation Panel -> 2D Annotations -> Add Local -> pt_juice.bed`. An example of using Juicebox to plot 3D domain boundary coordinates is shown in Figure 5.
...
![**Figure 5. Juicebox visualization of TAD boundaries called by Arrowhead (blue) vs predicted by *preciseTAD* (green).** The region presented are the genomic coordinates spanning 50,055,000-51,360,000 on chromosome 14 of GM12878.](figures/juicer_screenshot2.png){width=75%}
...
![**Figure 6. Levels of CTCF enrichment around *preciseTAD*-predicted boundaries are greater than Arrowhead-called boundaries.** ](figures/pt_vs_arrowhead.png)
...
![**Figure 7. *preciseTAD*-predicted TAD boundaries are more conserved across cell lines.** ](figures/conservation.png){width=75%}
...
![**Figure 8. Cross cell-line predicted boundaries strongly overlap with same-cell-line predicted boundaries.** ](figures/cross_cell_line_overlap.png){width=75%}
